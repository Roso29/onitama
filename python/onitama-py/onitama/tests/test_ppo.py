from onitama.rl import ACMaskedCNNPolicy, OnitamaEnv, RandomAgent, get_mask, actionToMove
...
class RLTest(unittest.TestCase):
    ...
    def test_error(self):
        ...
        with graph.as_default():
            ...
            policy = ACMaskedCNNPolicy(sess, observation_space, action_space, n_env, n_steps, 1)
    ...
    def test_masked_outputs(self):
        ...
        with graph.as_default():
            ...
            assert (np.where(policy.proba_step([obs]))[1][0] == allowed), "Got: {} expected: {}".format(
                np.where(policy.proba_step([obs]))[1][0], allowed)
            assert (policy.step([obs])[0] == allowed), "Got: {} expected: {}".format(policy.step([obs])[0], allowed)

    def test_with_env(self):
        env = OnitamaEnv(self.seed) 
        env.reset()
        ppo = PPO2(ACMaskedCNNPolicy, env)
    ...
    def test_policy_ac_with_env(self):
        ...
        ppo = PPO2(ACMaskedCNNPolicy, env)
        obs = env.reset()
        ac, _ = ppo.predict(obs, deterministic=False)
        env.step(ac)
    ...
    def test_policy_proba_with_env_step(self):
        ...
        for ac_flat in np.where(ppo.action_probability(obs))[0]:
